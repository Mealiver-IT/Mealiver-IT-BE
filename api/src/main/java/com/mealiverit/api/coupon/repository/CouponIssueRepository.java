package com.mealiverit.api.coupon.repository;
...
public interface CouponIssueRepository extends JpaRepository<CouponIssue, Long> {
    ...
    @Query("SELECT ci FROM CouponIssue ci WHERE ci.userId = :userId AND (" +
            "ci.status = com.mealiverit.api.coupon.CouponStatus.ISSUED " +
            "OR (ci.status = com.mealiverit.api.coupon.CouponStatus.USED AND ci.usedAt >= :cutoff) " +
            "OR (ci.status = com.mealiverit.api.coupon.CouponStatus.CANCELED AND ci.canceledAt >= :cutoff) " +
            "OR (ci.status = com.mealiverit.api.coupon.CouponStatus.EXPIRED AND ci.expiredAt >= :cutoff)" +
            ")")
