...
- (e) Redisson 분산락은 fencing token 부재 문제로, 락을 든 클라이언트가 응답이 늦어지면 락이 만료된 뒤에도 자신이 여전히 락을 쥐고 있다고 착각하고 write를 계속할 수 있습니다 — "정합성 보장"이 목적인 이 과제에는 구조적으로 안 맞는다고 판단해 기각했습니다.
...
Lua script 게이트가 "Redis 원자적 연산"이라는 요구조건을 가장 직관적으로 만족하는 방법이라 먼저 시도했습니다. 하지만 두 가지 리스크가 실제로 있던 사례들이 확인됐습니다:
1. Lua script 자체의 실행 오버헤드(올리브영 사례)
2. 복제 노드를 참조하면 정합성이 깨지는 반례(배민 빼빼로데이 사례)가 있음

이 버전을 완전히 건너뛰지 않고 "V2.0으로 일단 검토"하기로 한 이유는, "시도해봤고 이런 문제를 발견해서 다음 버전으로 넘어갔다"는 근거를 남기기 위해서입니다.
