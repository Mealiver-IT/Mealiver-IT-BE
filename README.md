...
**왜 처음부터 Redis로 안 가고 V1(DB 비관적 락)부터 시작했나**
과제의 평가 핵심은 "정확성"(초과발급 0건)이지 "처리량"이 아닙니다. Redis부터 붙이면 정확성과 인프라 복잡도를 동시에 떠안게 되므로, 먼저 MySQL만으로 가장 적은 코드로 "초과발급 0건"을 확실하게 증명하고, k6로 hot row 경합(지연시간 증가, 타임아웃)을 수치로 남겨서 "다음 버전이 왜 필요한지"의 근거로 쓰는 게 리스크가 가장 낮은 순서라고 판단했습니다.

**왜 V1에서 Redis(V2)로 넘어가나**
비관적 락은 재고 row 하나에 모든 요청이 몰리는 hot row 구조라, 락 대기 타임아웃·데드락·DB 커넥션 풀 고갈이라는 명확한 처리량 한계가 있습니다(V1 k6 결과로 실측 예정). Redis는 인메모리 단일 스레드 원자적 연산이라 DB 앞단에서 미리 걸러주기 때문에, DB에는 최대 재고 수량만큼만 요청이 도달하고 hot row 경합 자체가 사라집니다.

**왜 (b) 낙관적 락이나 (e) Redisson 분산락이 아니라 Redis 카운터 방식인가**
- (b) 낙관적 락은 경합이 심할수록 재시도가 폭증(thundering herd)해서 오히려 처리량이 나빠질 수 있고, (a)가 이미 같은 목표(정합성)를 더 단순하게 달성하므로 굳이 채택할 이유가 없었습니다.
- (e) Redisson 분산락은 Kleppmann(2016)이 지적한 fencing token 부재 문제로, 락을 든 클라이언트가 GC 정지 등으로 응답이 늦어지면 락이 만료된 뒤에도 자신이 여전히 락을 쥐고 있다고 착각하고 write를 계속할 수 있습니다 — "정합성 보장"이 목적인 이 과제에는 구조적으로 안 맞는다고 판단해 기각했습니다.
- (d) Redis+Kafka 비동기 분리는 eventual consistency(최종 정합성)를 받아들여야 하는데, 이 과제는 "즉시 정합성 검증"이 평가 포인트라 설명 부담이 크다고 보고 선택 확장으로 미뤘습니다.

**왜 V2.0(Redis Lua)을 거쳐서 V2.1(이중 카운터)로 갔나 — 처음부터 이중 카운터로 안 간 이유**
Lua script 게이트가 "Redis 원자적 연산"이라는 요구조건을 가장 직관적으로 만족하는 방법이라 먼저 시도했습니다. 하지만 두 가지 리스크가 실제로 확인됐습니다:
1. Lua script 자체의 실행 오버헤드(참고 사례 실측 -21%)
2. 복제 노드를 참조하면 정합성이 깨지는 반례(배민 빼빼로데이 사례)가 알려져 있음

이 버전을 완전히 건너뛰지 않고 "V2.0으로 일단 검토"한 이유는, "시도해봤고 이런 문제를 발견해서 다음 버전으로 넘어갔다"는 근거를 남기기 위해서입니다 — 곧바로 이중 카운터로 갔다면 왜 Lua를 배제했는지 설명할 실측 근거가 없었을 것입니다.

**왜 최종적으로 이중 카운터(f)를 채택했나**
`countReq`(1차 저비용 필터)와 `count`(2차 확정, 실패 시 자체 롤백)를 분리한 구조로, Lua 대비 오버헤드가 낮고(참고 사례 실측: Lua -21% vs 이중 카운터 -8%), 무엇보다 "`count`가 독립적으로 자체 롤백하기 때문에 총 발급량이 재고를 절대 못 넘는다"는 것을 논리적으로 증명할 수 있는 구조입니다. 다만 이 구조에도 트레이드오프가 있습니다 — 1인1매 위반이나 idempotency 충돌로 중간에 죽는 요청도 `countReq`는 이미 소모하기 때문에, 재고가 남았는데도 신규 유저가 조기에 "품절" 처리되는 UX 이슈가 있을 수 있습니다. 이건 초과발급 문제가 아니라 공정성 문제이고, 메인 부하테스트 시나리오(서로 다른 유저 20,000명, 재요청 없음)에서는 애초에 발현되지 않는 케이스라 감수하기로 했습니다.
