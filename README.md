...
| 역할 | 이름 | 담당 도메인 |
|---|---|---|
| 인프라·발급 API (발급 로직) | 김⁠어⁠진 | `CouponIssuanceService`, 재고 예약 전략(비관적 락 → Redis 스냅샷 캐시, [2026-08-20] 이중 카운터 계획에서 변경 → 같은 날 저녁 재고 샤딩 추가) |
...
| 분산 캐시 / 락 | ![Redis](https://img.shields.io/badge/Redis-7-DC382D?style=for-the-badge&logo=redis&logoColor=white) — DB 선반영+스냅샷 캐시 기반 사전필터 (구현·연동 완료, [2026-08-20] 이중 카운터 게이트 계획에서 변경). 재고 판단 자체는 [2026-08-20 저녁] MySQL `campaign_stock_shard` 샤딩으로 이관, Redis는 사전필터 캐시로만 계속 사용 |
...
- `Mealiver-IT-Infra` 레포에 `docker-compose.yml`로 인프라 전체 구성 완료. Redis는 재고 스냅샷 캐시(V3, [2026-08-20] 구현·연동 완료)로 사용 중입니다. 재고 판단의 원본은 [2026-08-20 저녁] MySQL `campaign_stock_shard`(V4, 재고 샤딩)로 옮겨갔고 Redis는 그 결과를 사후 복사하는 캐시 역할만 유지합니다. Kafka 컨테이너는 아직 애플리케이션에서 사용하지 않습니다(Phase 4 선택 확장).
...
| `campaign` | 선착순 발급 이벤트(캠페인) 마스터. 재고·오픈 기간·회원 등급 제한을 가짐 | `total_stock`/`remaining_stock`([2026-08-20 저녁 갱신] V4(재고 샤딩) 도입 이후 이 컬럼은 더 이상 재고 판단의 원본이 아님 — 진짜 재고는 `campaign_stock_shard` 합계이고, `remaining_stock`은 관리자 CRUD·검증쿼리(b) 등 기존 코드가 계속 읽을 수 있도록 스냅샷 리스너/재동기화 잡이 그 합계를 사후 복사해두는 표시값), `min_membership_tier`(nullable, 회원 전용 쿠폰 자격요건), `version`(낙관적 락) |
| `campaign_stock_shard` | [2026-08-20 저녁 신설] 캠페인 재고를 N개(기본 10) row로 쪼갠 실제 재고 저장 단위. InnoDB가 서로 다른 샤드를 독립적으로 잠글 수 있게 해 hot row 경합을 분산 | `campaign_id`+`shard_index`(UNIQUE), `remaining_stock`, `capacity`(샤드 생성 시점 값으로 고정, `sum(capacity)=total_stock` 불변식) |
...
> **[2026-08-20 오전 갱신]** 8/19 실측 부하테스트(`coupon_mixed_5k_x4.js`, 캠페인 300개에
...
> V3(DB 선반영 + Redis 스냅샷 캐시) 하나로 단순화했습니다.
>
> **[2026-08-20 저녁 갱신]** V3까지 적용한 뒤 부하테스트를 이어가 보니, 목표 달성률이
> 80%대에서 정체됐습니다 — hot row(campaign 1개 row)로 몰리는 처리량 한계 자체는
> 안 풀렸던 겁니다. 그래서 재고를 여러 row(샤드)로 쪼개는 **V4(재고 샤딩)**를 같은 날
> 저녁에 추가로 구현·병합했습니다(PR #100, 김어진). V4는 옛 계획의 "V4=DB 백스톱"과는
> 이름만 같고 전혀 다른 내용입니다. 아래는 두 차례 갱신을 모두 반영한 최신 버전입니다 —
> 이전 계획과 비교한 트레이드오프는
> [`03_버전사다리_실험설계.txt`](docs/planning/03_버전사다리_실험설계.txt) 참고.

재고 초과 발급을 막기 위해, 서로 다른 축에서 실패(또는 극복)하도록 설계한 4단계 버전 사다리(V1~V4)로 동시성 제어 전략을 비교합니다.
| 전략 | 정합성 | 처리량 | 인프라 의존성 |
|---|---|---|---|
...
| (g) DB 선반영 + Redis 스냅샷 캐시(사전필터) | (a)와 동일(강함) — Redis는 발급을 절대 결정하지 않음 | (a) 대비 개선(품절 이후 요청이 DB에 도달하지 않음) | Redis 있으면 가속, 없어도 (a)로 자동 폴백 |
| **(h) 재고 샤딩** (campaign 1 row → N개 row 분할) | (a)/(g)와 동일(강함) — 판정은 여전히 DB, 샤드 단위 원자 UPDATE | (g) 대비 추가 개선(재고가 남아있는 동안의 순수 경합도 분산) | MySQL만(Redis는 (g) 사전필터 유지용) — **최종 채택** |

여러 후보 중 (b) 낙관적 락은 (a)가 같은 목표를 더 단순하게 달성해 채택하지 않았고, (e) Redisson 분산락은 fencing token 부재로 정합성 목적에 안 맞아 기각, (d) Kafka 비동기 분리는 eventual consistency가 "즉시 정합성 검증" 평가 포인트와 맞지 않아 선택 확장으로 남겨뒀습니다. (c)/(f)는 둘 다 "Redis가 발급을 직접 결정"하는 구조라 Redis 상태 유실 시 별도 DB 백스톱(V4)이 반드시 필요했는데, 8/19 실측으로 드러난 진짜 문제(품절 이후에도 요청이 DB까지 도달해 커넥션 풀을 갉아먹는 것)에는 정확도가 아니라 "불필요한 DB 진입을 막는 것"이 필요하다는 결론에 도달해 (g)를 채택했습니다. 다만 (g)는 "품절 이후"만 줄일 뿐 "재고가 남아있는 동안의 순수 경합"은 그대로였고, 이게 8/20 저녁 실측(목표 달성률 80%대 정체)의 원인이었습니다. 그래서 hot row 자체를 여러 row로 쪼개는 (h)를 추가 채택했습니다.
```mermaid
flowchart LR
    V1["V1<br/>제어 없음<br/>(미구현)"] --> V2["V2<br/>DB 비관적 락<br/>(a)"] --> V3["V3<br/>DB 선반영 +<br/>Redis 스냅샷 캐시<br/>(g)"] --> V4["V4<br/>재고 샤딩<br/>(h)"]
```
...
- **V2 — DB 비관적 락 (`PessimisticLockStockReservationStrategy`)**: 재고 row를 잠그고 확인·차감하는, 별도 인프라 없이 정확성을 확보하는 방식입니다. 재고 row 하나에 요청이 몰리는 hot row 구조라 락 대기 타임아웃 등 처리량 한계가 명확히 드러납니다(8/19 `coupon_mixed_5k_x4.js` 실측: 목표 5,000건 중 2,652건만 발급, DB 커넥션 풀 고갈). 8/20 오후에는 "조건 확인 + 차감"을 단일 원자 UPDATE로 묶어 락 보유 구간을 트랜잭션 전체에서 UPDATE 문 하나로 좁히는 개선이 한 번 더 들어갔습니다(여전히 V2 — 전략이 아니라 구현 최적화).
- **V3 — DB 선반영 + Redis 스냅샷 캐시**: 최종 판단은 항상 그 시점의 활성 전략(V2 또는 V4)이 하고, Redis는 그 결과를 `AFTER_COMMIT` 시점에 사후 복사해두는 캐시입니다. 발급 요청은 DB에 가기 전 Redis 스냅샷을 먼저 조회해 "확실히 품절"이면 즉시 거절하고, 그 외(캐시 미스·Redis 장애 포함)는 전부 DB 경로로 통과시킵니다. `CampaignStockSnapshotReconciliationJob`이 15초 주기로 DB 기준값을 다시 써서 스냅샷 드리프트를 자가치유합니다(TTL 60초는 이 잡이 멈췄을 때만 발동하는 최후 안전장치). Redis가 죽어도 정확성(초과발급 0건)은 그대로 유지되고, 사전필터 효과만 사라져 지연시간이 사전필터 없는 수준으로 되돌아갑니다 — 구 V4(DB 백스톱)가 하려던 일을 별도 단계 없이 설계 자체로 만족합니다.
- **V4 — 재고 샤딩 (`ShardedStockReservationStrategy`)**: `campaign_stock_shard` 테이블로 재고를 N개(기본 10) row로 쪼개, InnoDB가 서로 다른 샤드는 동시에 잠글 수 있게 합니다. 요청마다 랜덤 시작 샤드 + 순차 폴백으로 배정하므로 특정 샤드가 비어도 다른 샤드에 재고가 남아있으면 정상 처리됩니다. 샤드는 캠페인 생성 시점이 아니라 이 전략이 그 캠페인을 처음 다룰 때 지연 생성되어(`campaign.remaining_stock` 기준), 기존 캠페인도 별도 백필 없이 자동으로 채워집니다. 여러 샤드를 순회하는 시도는 `REQUIRES_NEW`로 완전히 독립된 트랜잭션이라 락을 오래 들고 있지 않으며, 서로 다른 순서로 여러 샤드를 방문하는 요청끼리의 데드락도 방지합니다(100 동시요청 테스트로 실측 확인). `rollback()`은 원래 감소시켰던 그 샤드가 아니라 여유 있는 아무 샤드에나 복원하는데, 각 샤드의 capacity가 생성 시점 값으로 고정돼 `sum(capacity)=total_stock`이 항상 성립하므로 정확성에 문제가 없습니다. V3의 Redis 사전필터는 코드 변경 없이 V4 위에도 그대로 얹힙니다(스냅샷 소스만 `campaign.remaining_stock`에서 샤드 합계로 교체).
`StockReservationStrategy` 인터페이스로 재고 확보 로직(`reserve`/`rollback`)만 버전별로 분리하고, 멱등성·eligibility 체크·트랜잭션 경계·로깅 등 나머지 요소는 전 버전에서 동일하게 유지해 버전 간 비교가 공정하도록 설계했습니다(V4에서도 인터페이스 시그니처 변경 없음). V3는 별도 구현체가 아니라 그 시점의 활성 전략 재사용 + 서비스 계층의 Redis 사전필터 조합입니다. 자세한 비교·근거는 [`03_버전사다리_실험설계.txt`](docs/planning/03_버전사다리_실험설계.txt), 설계 배경은 [`04_아키텍처.md`](docs/planning/04_아키텍처.txt) 4절 참고.
### 왜 이렇게 결정했나
| 결정 | 이유 |
|---|---|
...
| **V3 → V4(재고 샤딩) [2026-08-20 저녁]** | Redis 사전필터는 품절 이후의 낭비 트래픽만 줄일 뿐, 재고가 남아있는 동안의 순수 경합 자체는 못 줄임 — 그게 목표 달성률 80%대 정체의 원인이었음. hot row 자체를 N개로 분산해 처리량 한계를 구조적으로 해소 |
...
**부하테스트**: k6로 재고 대비 2배 요청(재고 10,000 / 서로 다른 유저 20,000명, ramp-up 60초 — 멘토 확정 공식 조건)을 걸어 초과발급 0건과 1인 1매를 검증하고, 동일 Idempotency-Key로 동시 재요청을 반복 전송해 상태가 정확히 1회만 변경되는지 검증합니다(`api/src/test/K6/phase1/`). `coupon_mixed_5k_x4.js`(8/19~8/20 실패 재현 시나리오)는 아직 저장소에 커밋되지 않아, 버전 간 개선폭을 실측으로 남기려면 커밋과 재실행이 필요합니다.
...
- **발급**: 클라이언트가 매 요청마다 `Idempotency-Key` 헤더를 전송, `coupon_issue.idempotency_key`의 UNIQUE 제약으로 동일 키 재요청을 DB가 거부. 재요청 시 created인 `201`이 아닌 OK `200`과 기존 발급 결과를 그대로 반환 받아 새로 발급이 아닌 재사용이라는 것을 알 수 있음 (k6 리허설 스펙으로 검증 완료). 재고 확보는 그 시점의 활성 전략(V2 DB 비관적 락 → [2026-08-20 저녁] V4 재고 샤딩)이 담당하며, Redis 도입(V3, [2026-08-20 오전]) 이후에도 이 역할은 그대로 DB가 유지한다 — Redis는 사전필터 캐시로만 추가됨.
