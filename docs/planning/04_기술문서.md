...
관련: [01_기획서.md](./01_기획서.md) | [02_아키텍처.md](./02_아키텍처.md) | [03_시스템설계.md](./03_시스템설계.md) | [06_일정과역할.md](./06_일정과역할.md) | [05_개발표준.md](./05_개발표준.md)
...
| DB | MySQL | 확정 |
| 캐시/원자연산 | Redis | 하드닝 단계 동시성 게이트키퍼용 (확정, 전략 f — 이중 카운터, V2.1) |
| 메시지 큐 | Kafka (선택) | Phase 4 선택 확장에서만 검토 |
| 인프라 | 앱은 로컬 Docker Compose(MySQL, Redis, 선택적 Kafka), DB는 Tailscale 경유 원격 호스트(`100.125.247.64:3306`) | 확정 |
| 마이그레이션 | Flyway | `V1__create_core_tables.sql` 기준 |
| 배치 | Spring Batch | 정합성 검증 Job, 300만 건 chunk 처리 |
| 재시도 | spring-retry | 낙관적 락 충돌 시 재시도 |
| 분산락 | Redisson | 검토 후 기각([02_아키텍처.md](./02_아키텍처.md) 4.1절, Kleppmann fencing token 근거) — 미사용 |
...
- `src/main/resources/db/migration/V2__create_order_tier_tables.sql` — Orders/MembershipTierLog 스키마 ([02_아키텍처.md](./02_아키텍처.md) 1절)
...
- `src/main/java/.../batch/ConsistencyVerificationJob.java` — 정합성 검증 Spring Batch Job ([03_시스템설계.md](./03_시스템설계.md) 1절)
- `src/main/java/.../membership/batch/MembershipTierBatchJob.java` — 매월 1일 계급 재산정 배치 ([02_아키텍처.md](./02_아키텍처.md) 6.2절)
- `src/test/loadtest/coupon_race.js` — k6 부하테스트 스크립트
- `docker-compose.yml` — MySQL/Redis/Kafka 로컬 인프라 정의

도메인 모델과 코드 패턴(엔티티/서비스/상태머신) 상세는 [02_아키텍처.md](./02_아키텍처.md) 참고. 정합성 검증 SQL/Batch, 더미데이터 생성, PII 마스킹, 부하테스트 스크립트 상세는 [03_시스템설계.md](./03_시스템설계.md) 참고.
...
| 부하 테스트 | 10,000 vs 20,000 시나리오 | k6 | [03_시스템설계.md](./03_시스템설계.md) 5절 지표 |
...
ECC `testing.md`/`springboot-tdd` 기준 커버리지 80%+를 목표로 하되, 이 프로젝트는 커버리지 수치보다 **동시성 시나리오 테스트의 실효성**(실제로 경쟁 상황을 재현하는가)이 더 중요한 평가 축임에 유의. 코딩 표준·보안 체크리스트·검증 루프 상세는 [05_개발표준.md](./05_개발표준.md) 참고.
