...
- Redis-DB 간 정합성 어긋남은 보상 로직(재고 복구 INCR) + 검증 배치([03_시스템설계.md](./03_시스템설계.md) 참고)로 이중 방어.
...
**확정: MVP=(a) 비관적 락 → 하드닝=(c) Redis Lua script 조합으로 채택.**
...
public IssueResult issue(Long userId, Long campaignId, String idempotencyKey) {
    ...
    // 3) 재고 확보 (전략에 따라 Redis Lua / DB 비관적락)
...
**동시성 전략과의 관계**: eligibility 체크는 5.1절 `issue()`의 2단계로, 재고 확보(락 획득) *이전에* 수행한다. 따라서 등급 미달 요청은 락 경합에 아예 진입하지 않고 즉시 거부되어, 전략 (a)/(c) 어느 쪽을 쓰든 동시성 설계에 영향을 주지 않는다. `campaign.min_membership_tier`는 캠페인 생성 시 고정되는 값이라 `campaign` row 자체에 캐시해도 되고(위 스니펫), Redis 게이트(전략 c) 채택 시에는 Lua script 진입 전 애플리케이션 레벨에서 먼저 걸러도 된다.
