...
User (mock, 회원가입/로그인 없음)
 ...
 ├─ membership_tier (ENUM: PRIVATE, PFC, CORPORAL, SERGEANT, DEFAULT PRIVATE)  -- 이병/일병/상병/병장, 더미데이터 생성 시 가중 랜덤 분포로 직접 부여 (01_기획서.md 6.2절 — Order 테이블 없어 주문건수 기준 불가)
 └─ created_at
...
Coupon (캠페인이 발급하는 쿠폰 정책 마스터. 캠페인:쿠폰 1:1로 단순화 가능)
 ...
 ├─ discount_value (DECIMAL)   -- RATE 타입 + 계급별 차등 캠페인의 경우 기본값(이병/일병 기준). 실제 적용률은 TierDiscountPolicy(6절) 참고
